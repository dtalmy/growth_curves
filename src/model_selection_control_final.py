...
Adapted by @michaelxlin for Controls
'''
...
Allfiles = glob.glob(path+'*Control*.txt') # all files
files = glob.glob(path+'[!Std]*Control*.txt') # NOT standard deviations
stddevfiles = glob.glob(path+'StdDev*Control*.txt') # standard deviations
...
labs = ['M11','M12'] # Simplest Infection 
...
for (inf,tag) in zip(exp_set_all, vals_all):
    ...
    #affs = r_[[pmod.pms['aff'] for pmod in allmods]]
    ...
    #affss = r_[[pmod.pss['aff'] for pmod in allmods]]
